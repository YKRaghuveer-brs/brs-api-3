...
const displayCharacters = (characters) => {
    if (characters.length === 0) {
        charactersList.innerHTML = '<li class="character">NO RESULTS FOUND</li>';
    } else {
        const htmlString = characters
            .map((character) => {
                return `
                <li class="character">
                    ...
                    <p id="ancestry">Ancestry: ${character.ancestry}</p>
                    <p id="dateOfBirth">Date of Birth: ${character.dateOfBirth}</p>
                    <p id="dateOfBirth">Image: ${character.image}</p>
                    <img src="${character.image || no_image_available}" alt="${character.name}"></img>
                </li>
            `;
            })
            .join('');
